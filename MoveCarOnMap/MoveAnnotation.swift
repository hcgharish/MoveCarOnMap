...
let seconds = seconds_1 * 60
let sleepTime = seconds / 1
let sleepTimeForSender = sleepTime + 1
...
class MoveAnnotation:NSObject {
    ...
    var maPaths: NSMutableArray? = NSMutableArray()
    
    var receiverLastLocation:CLLocationCoordinate2D? = nil
    ...
    @objc func moveAnnotation () {
        DispatchQueue.global().async {
            while (true) {
                ...
                if let ano = self.ano, let count = self.maPaths?.count, count > 0, let ob = self.maPaths?[0] as? CalculatePathReturn {
                        self.maPaths?.removeObject(at: 0)
                        
                        if let points = ob.points, let dist = ob.dist, points.count > 0, dist > 0 {
                            let count = points.count
                            
                            //let sleepFor = Double(sleepTime) / Double(count * 8)
                            let sleepFor = Double(sleepTime) / Double(count * 1)
                            boolWait = false
                            
                            for i in 0..<Int(count) {
                                
                                let loc = CLLocationCoordinate2DMake((points[i][0]), (points[i][1]))
                                
                                if loc.latitude != 0.0 && loc.longitude != 0.0 {
                                    //DispatchQueue.global().async {
                                        self.animateNow (ano, loc, sleepFor)
                                    //}
                                }
            ...
            while (true) {
                ...
                if self.ano != nil {
                    ...
                    if self.maPaths!.count > 0 {
                        var ob = self.maPaths![0] as? CalculatePathReturn
                            ...
                            if ob?.points != nil {
                                if (ob?.points?.count)! > 0 {
                                    if (ob?.dist)! > 0.0 {
                                        ...
                                        //let sleepFor = Double(sleepTime) / Double((ob?.points?.count)! * 8)
                                        let sleepFor = Double(sleepTime) / Double((ob?.points?.count)! * 1)
   ...
   // weak var vcMapTracking: MapTracking?
    ...
    func animateNow (_ ano:MyAnnotation, _ point:CLLocationCoordinate2D, _ slee:Double) {
        ...
        DispatchQueue.main.async {
            ...
            if self.receiverLastLocation != nil {
                let coords = UnsafeMutablePointer<CLLocationCoordinate2D>.allocate(capacity: 2)
                
                coords[0] = self.receiverLastLocation!
                coords[1] = CLLocationCoordinate2D(latitude: point.latitude, longitude: point.longitude)
    ...
    deinit {
        ...
        id = nil
        maPaths = nil
        receiverLastLocation = nil
        loc_old = nil
        poliLine = nil
        //vcMapTracking = nil
        lastAnnotationCLLocation = nil
    }
}
